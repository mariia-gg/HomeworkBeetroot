...
namespace Text
{
    class PhonesStorage
    {
        public readonly string DbFilePath = @"C:\PhoneDbFolder\phonedb.csv";
        ...
        public void Sort()
        {
            var lines = System.IO.File.ReadLines(@"C:\PhoneDbFolder\phonedb.csv");
            var linesOrderedBySurname = lines.OrderBy((p) => p.Split(',')[1]).ToList();
            foreach (var line in linesOrderedBySurname)
            {
